...
class HybridRecommender:
    ...
    def recommend_top_n(
        self, user_id: int, n: int = 10, filtered_watched: bool = True
    ) -> pd.DataFrame:

        scores_a = self.model_a.recommend_top_n(
            user_id=user_id, n=len(self.movies), filtered_watched=False
        )
        ...
        recommendations = combined.merge(
            self.movies[["movieId", "title"]], on="movieId", how="left"
        )
